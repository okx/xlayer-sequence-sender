// Code generated - DO NOT EDIT.
// This file is a generated binding and any manual changes will be lost.

package polygonzkevmfeijoa

import (
	"errors"
	"math/big"
	"strings"

	ethereum "github.com/ethereum/go-ethereum"
	"github.com/ethereum/go-ethereum/accounts/abi"
	"github.com/ethereum/go-ethereum/accounts/abi/bind"
	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/core/types"
	"github.com/ethereum/go-ethereum/event"
)

// Reference imports to suppress errors if they are not otherwise used.
var (
	_ = errors.New
	_ = big.NewInt
	_ = strings.NewReader
	_ = ethereum.NotFound
	_ = bind.Bind
	_ = common.Big1
	_ = types.BloomLookup
	_ = event.NewSubscription
	_ = abi.ConvertType
)

// PolygonRollupBaseFeijoaBlobData is an auto generated low-level Go binding around an user-defined struct.
type PolygonRollupBaseFeijoaBlobData struct {
	BlobType       uint8
	BlobTypeParams []byte
}

// PolygonzkevmfeijoaMetaData contains all meta data concerning the Polygonzkevmfeijoa contract.
var PolygonzkevmfeijoaMetaData = &bind.MetaData{
	ABI: "[{\"inputs\":[{\"internalType\":\"contractIPolygonZkEVMGlobalExitRootV2\",\"name\":\"_globalExitRootManager\",\"type\":\"address\"},{\"internalType\":\"contractIERC20Upgradeable\",\"name\":\"_pol\",\"type\":\"address\"},{\"internalType\":\"contractIPolygonZkEVMBridgeV2\",\"name\":\"_bridgeAddress\",\"type\":\"address\"},{\"internalType\":\"contractPolygonRollupManager\",\"name\":\"_rollupManager\",\"type\":\"address\"}],\"stateMutability\":\"nonpayable\",\"type\":\"constructor\"},{\"inputs\":[],\"name\":\"BlobHashNotFound\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"BlobTypeNotSupported\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"FinalAccInputHashDoesNotMatch\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ForceBlobNotAllowed\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ForceBlobTimeoutNotExpired\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ForceBlobsAlreadyActive\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ForceBlobsDecentralized\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ForceBlobsNotAllowedOnEmergencyState\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ForceBlobsOverflow\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ForcedDataDoesNotMatch\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"GasTokenNetworkMustBeZeroOnEther\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"HaltTimeoutNotExpiredAfterEmergencyState\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"HugeTokenMetadataNotSupported\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"InvalidCommitmentAndProofLength\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"InvalidInitializeTransaction\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"InvalidRangeForceBlobTimeout\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"Invalidl1InfoLeafIndex\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"MaxTimestampSequenceInvalid\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"NotEnoughMaticAmount\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"NotEnoughPOLAmount\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"OnlyAdmin\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"OnlyPendingAdmin\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"OnlyRollupManager\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"OnlyTrustedSequencer\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"PointEvalutionPrecompiledFail\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"SequenceZeroBlobs\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"SequencedTimestampBelowForcedTimestamp\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"TransactionsLengthAboveMax\",\"type\":\"error\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"address\",\"name\":\"newAdmin\",\"type\":\"address\"}],\"name\":\"AcceptAdminRole\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"uint64\",\"name\":\"forceBlobNum\",\"type\":\"uint64\"},{\"indexed\":false,\"internalType\":\"bytes32\",\"name\":\"lastGlobalExitRoot\",\"type\":\"bytes32\"},{\"indexed\":false,\"internalType\":\"address\",\"name\":\"sequencer\",\"type\":\"address\"},{\"indexed\":false,\"internalType\":\"uint64\",\"name\":\"zkGasLimit\",\"type\":\"uint64\"},{\"indexed\":false,\"internalType\":\"bytes\",\"name\":\"transactions\",\"type\":\"bytes\"}],\"name\":\"ForceBlob\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"bytes\",\"name\":\"transactions\",\"type\":\"bytes\"},{\"indexed\":false,\"internalType\":\"bytes32\",\"name\":\"lastGlobalExitRoot\",\"type\":\"bytes32\"},{\"indexed\":false,\"internalType\":\"address\",\"name\":\"sequencer\",\"type\":\"address\"}],\"name\":\"InitialSequenceBlobs\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"uint8\",\"name\":\"version\",\"type\":\"uint8\"}],\"name\":\"Initialized\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"uint64\",\"name\":\"lastBlobSequenced\",\"type\":\"uint64\"}],\"name\":\"SequenceBlobs\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"uint64\",\"name\":\"numBlob\",\"type\":\"uint64\"}],\"name\":\"SequenceForceBlobs\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"address\",\"name\":\"newForceBlobAddress\",\"type\":\"address\"}],\"name\":\"SetForceBlobAddress\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"uint64\",\"name\":\"newforceBlobTimeout\",\"type\":\"uint64\"}],\"name\":\"SetForceBlobTimeout\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"string\",\"name\":\"newNetworkName\",\"type\":\"string\"}],\"name\":\"SetNetworkName\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"address\",\"name\":\"newTrustedSequencer\",\"type\":\"address\"}],\"name\":\"SetTrustedSequencer\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"string\",\"name\":\"newTrustedSequencerURL\",\"type\":\"string\"}],\"name\":\"SetTrustedSequencerURL\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"address\",\"name\":\"newPendingAdmin\",\"type\":\"address\"}],\"name\":\"TransferAdminRole\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"uint64\",\"name\":\"sequneceNum\",\"type\":\"uint64\"},{\"indexed\":false,\"internalType\":\"bytes32\",\"name\":\"stateRoot\",\"type\":\"bytes32\"},{\"indexed\":true,\"internalType\":\"address\",\"name\":\"aggregator\",\"type\":\"address\"}],\"name\":\"VerifyBlobs\",\"type\":\"event\"},{\"inputs\":[],\"name\":\"GLOBAL_EXIT_ROOT_MANAGER_L2\",\"outputs\":[{\"internalType\":\"contractIBasePolygonZkEVMGlobalExitRoot\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"INITIALIZE_TX_BRIDGE_LIST_LEN_LEN\",\"outputs\":[{\"internalType\":\"uint8\",\"name\":\"\",\"type\":\"uint8\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"INITIALIZE_TX_BRIDGE_PARAMS\",\"outputs\":[{\"internalType\":\"bytes\",\"name\":\"\",\"type\":\"bytes\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"INITIALIZE_TX_BRIDGE_PARAMS_AFTER_BRIDGE_ADDRESS\",\"outputs\":[{\"internalType\":\"bytes\",\"name\":\"\",\"type\":\"bytes\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"INITIALIZE_TX_BRIDGE_PARAMS_AFTER_BRIDGE_ADDRESS_EMPTY_METADATA\",\"outputs\":[{\"internalType\":\"bytes\",\"name\":\"\",\"type\":\"bytes\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"INITIALIZE_TX_CONSTANT_BYTES\",\"outputs\":[{\"internalType\":\"uint16\",\"name\":\"\",\"type\":\"uint16\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"INITIALIZE_TX_CONSTANT_BYTES_EMPTY_METADATA\",\"outputs\":[{\"internalType\":\"uint16\",\"name\":\"\",\"type\":\"uint16\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"INITIALIZE_TX_DATA_LEN_EMPTY_METADATA\",\"outputs\":[{\"internalType\":\"uint8\",\"name\":\"\",\"type\":\"uint8\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"INITIALIZE_TX_EFFECTIVE_PERCENTAGE\",\"outputs\":[{\"internalType\":\"bytes1\",\"name\":\"\",\"type\":\"bytes1\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"MAX_SEQUENCE_TIMESTAMP_FORCED\",\"outputs\":[{\"internalType\":\"uint64\",\"name\":\"\",\"type\":\"uint64\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"POINT_EVALUATION_PRECOMPILE_ADDRESS\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"SIGNATURE_INITIALIZE_TX_R\",\"outputs\":[{\"internalType\":\"bytes32\",\"name\":\"\",\"type\":\"bytes32\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"SIGNATURE_INITIALIZE_TX_S\",\"outputs\":[{\"internalType\":\"bytes32\",\"name\":\"\",\"type\":\"bytes32\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"SIGNATURE_INITIALIZE_TX_V\",\"outputs\":[{\"internalType\":\"uint8\",\"name\":\"\",\"type\":\"uint8\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"TIMESTAMP_RANGE\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"ZK_GAS_LIMIT_BATCH\",\"outputs\":[{\"internalType\":\"uint64\",\"name\":\"\",\"type\":\"uint64\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"acceptAdminRole\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"admin\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"bridgeAddress\",\"outputs\":[{\"internalType\":\"contractIPolygonZkEVMBridgeV2\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"calculatePolPerForcedZkGas\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"bytes\",\"name\":\"blobData\",\"type\":\"bytes\"},{\"internalType\":\"uint256\",\"name\":\"polAmount\",\"type\":\"uint256\"}],\"name\":\"forceBlob\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"forceBlobAddress\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"forceBlobTimeout\",\"outputs\":[{\"internalType\":\"uint64\",\"name\":\"\",\"type\":\"uint64\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint64\",\"name\":\"\",\"type\":\"uint64\"}],\"name\":\"forcedBlobs\",\"outputs\":[{\"internalType\":\"bytes32\",\"name\":\"hashedForcedBlobData\",\"type\":\"bytes32\"},{\"internalType\":\"uint64\",\"name\":\"forcedTimestamp\",\"type\":\"uint64\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"gasTokenAddress\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"gasTokenNetwork\",\"outputs\":[{\"internalType\":\"uint32\",\"name\":\"\",\"type\":\"uint32\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint32\",\"name\":\"networkID\",\"type\":\"uint32\"},{\"internalType\":\"address\",\"name\":\"_gasTokenAddress\",\"type\":\"address\"},{\"internalType\":\"uint32\",\"name\":\"_gasTokenNetwork\",\"type\":\"uint32\"},{\"internalType\":\"bytes\",\"name\":\"_gasTokenMetadata\",\"type\":\"bytes\"}],\"name\":\"generateInitializeTransaction\",\"outputs\":[{\"internalType\":\"bytes\",\"name\":\"\",\"type\":\"bytes\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"globalExitRootManager\",\"outputs\":[{\"internalType\":\"contractIPolygonZkEVMGlobalExitRootV2\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"_admin\",\"type\":\"address\"},{\"internalType\":\"address\",\"name\":\"sequencer\",\"type\":\"address\"},{\"internalType\":\"uint32\",\"name\":\"networkID\",\"type\":\"uint32\"},{\"internalType\":\"address\",\"name\":\"_gasTokenAddress\",\"type\":\"address\"},{\"internalType\":\"string\",\"name\":\"sequencerURL\",\"type\":\"string\"},{\"internalType\":\"string\",\"name\":\"_networkName\",\"type\":\"string\"}],\"name\":\"initialize\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"lastAccInputHash\",\"outputs\":[{\"internalType\":\"bytes32\",\"name\":\"\",\"type\":\"bytes32\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"lastForceBlob\",\"outputs\":[{\"internalType\":\"uint64\",\"name\":\"\",\"type\":\"uint64\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"lastForceBlobSequenced\",\"outputs\":[{\"internalType\":\"uint64\",\"name\":\"\",\"type\":\"uint64\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"networkName\",\"outputs\":[{\"internalType\":\"string\",\"name\":\"\",\"type\":\"string\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint64\",\"name\":\"lastVerifiedSequenceNum\",\"type\":\"uint64\"},{\"internalType\":\"bytes32\",\"name\":\"newStateRoot\",\"type\":\"bytes32\"},{\"internalType\":\"address\",\"name\":\"aggregator\",\"type\":\"address\"}],\"name\":\"onVerifySequences\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"pendingAdmin\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"pol\",\"outputs\":[{\"internalType\":\"contractIERC20Upgradeable\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"rollupManager\",\"outputs\":[{\"internalType\":\"contractPolygonRollupManager\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"components\":[{\"internalType\":\"uint8\",\"name\":\"blobType\",\"type\":\"uint8\"},{\"internalType\":\"bytes\",\"name\":\"blobTypeParams\",\"type\":\"bytes\"}],\"internalType\":\"structPolygonRollupBaseFeijoa.BlobData[]\",\"name\":\"blobs\",\"type\":\"tuple[]\"},{\"internalType\":\"address\",\"name\":\"l2Coinbase\",\"type\":\"address\"},{\"internalType\":\"bytes32\",\"name\":\"finalAccInputHash\",\"type\":\"bytes32\"}],\"name\":\"sequenceBlobs\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"components\":[{\"internalType\":\"uint8\",\"name\":\"blobType\",\"type\":\"uint8\"},{\"internalType\":\"bytes\",\"name\":\"blobTypeParams\",\"type\":\"bytes\"}],\"internalType\":\"structPolygonRollupBaseFeijoa.BlobData[]\",\"name\":\"blobs\",\"type\":\"tuple[]\"}],\"name\":\"sequenceForceBlobs\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"newForceBlobAddress\",\"type\":\"address\"}],\"name\":\"setForceBlobAddress\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint64\",\"name\":\"newforceBlobTimeout\",\"type\":\"uint64\"}],\"name\":\"setForceBlobTimeout\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"string\",\"name\":\"newNetworkName\",\"type\":\"string\"}],\"name\":\"setNetworkName\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"newTrustedSequencer\",\"type\":\"address\"}],\"name\":\"setTrustedSequencer\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"string\",\"name\":\"newTrustedSequencerURL\",\"type\":\"string\"}],\"name\":\"setTrustedSequencerURL\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"newPendingAdmin\",\"type\":\"address\"}],\"name\":\"transferAdminRole\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"trustedSequencer\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"trustedSequencerURL\",\"outputs\":[{\"internalType\":\"string\",\"name\":\"\",\"type\":\"string\"}],\"stateMutability\":\"view\",\"type\":\"function\"}]",
	Bin: "",
}

// PolygonzkevmfeijoaABI is the input ABI used to generate the binding from.
// Deprecated: Use PolygonzkevmfeijoaMetaData.ABI instead.
var PolygonzkevmfeijoaABI = PolygonzkevmfeijoaMetaData.ABI

// PolygonzkevmfeijoaBin is the compiled bytecode used for deploying new contracts.
// Deprecated: Use PolygonzkevmfeijoaMetaData.Bin instead.
var PolygonzkevmfeijoaBin = PolygonzkevmfeijoaMetaData.Bin

// DeployPolygonzkevmfeijoa deploys a new Ethereum contract, binding an instance of Polygonzkevmfeijoa to it.
func DeployPolygonzkevmfeijoa(auth *bind.TransactOpts, backend bind.ContractBackend, _globalExitRootManager common.Address, _pol common.Address, _bridgeAddress common.Address, _rollupManager common.Address) (common.Address, *types.Transaction, *Polygonzkevmfeijoa, error) {
	parsed, err := PolygonzkevmfeijoaMetaData.GetAbi()
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	if parsed == nil {
		return common.Address{}, nil, nil, errors.New("GetABI returned nil")
	}

	address, tx, contract, err := bind.DeployContract(auth, *parsed, common.FromHex(PolygonzkevmfeijoaBin), backend, _globalExitRootManager, _pol, _bridgeAddress, _rollupManager)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	return address, tx, &Polygonzkevmfeijoa{PolygonzkevmfeijoaCaller: PolygonzkevmfeijoaCaller{contract: contract}, PolygonzkevmfeijoaTransactor: PolygonzkevmfeijoaTransactor{contract: contract}, PolygonzkevmfeijoaFilterer: PolygonzkevmfeijoaFilterer{contract: contract}}, nil
}

// Polygonzkevmfeijoa is an auto generated Go binding around an Ethereum contract.
type Polygonzkevmfeijoa struct {
	PolygonzkevmfeijoaCaller     // Read-only binding to the contract
	PolygonzkevmfeijoaTransactor // Write-only binding to the contract
	PolygonzkevmfeijoaFilterer   // Log filterer for contract events
}

// PolygonzkevmfeijoaCaller is an auto generated read-only Go binding around an Ethereum contract.
type PolygonzkevmfeijoaCaller struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// PolygonzkevmfeijoaTransactor is an auto generated write-only Go binding around an Ethereum contract.
type PolygonzkevmfeijoaTransactor struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// PolygonzkevmfeijoaFilterer is an auto generated log filtering Go binding around an Ethereum contract events.
type PolygonzkevmfeijoaFilterer struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// PolygonzkevmfeijoaSession is an auto generated Go binding around an Ethereum contract,
// with pre-set call and transact options.
type PolygonzkevmfeijoaSession struct {
	Contract     *Polygonzkevmfeijoa // Generic contract binding to set the session for
	CallOpts     bind.CallOpts       // Call options to use throughout this session
	TransactOpts bind.TransactOpts   // Transaction auth options to use throughout this session
}

// PolygonzkevmfeijoaCallerSession is an auto generated read-only Go binding around an Ethereum contract,
// with pre-set call options.
type PolygonzkevmfeijoaCallerSession struct {
	Contract *PolygonzkevmfeijoaCaller // Generic contract caller binding to set the session for
	CallOpts bind.CallOpts             // Call options to use throughout this session
}

// PolygonzkevmfeijoaTransactorSession is an auto generated write-only Go binding around an Ethereum contract,
// with pre-set transact options.
type PolygonzkevmfeijoaTransactorSession struct {
	Contract     *PolygonzkevmfeijoaTransactor // Generic contract transactor binding to set the session for
	TransactOpts bind.TransactOpts             // Transaction auth options to use throughout this session
}

// PolygonzkevmfeijoaRaw is an auto generated low-level Go binding around an Ethereum contract.
type PolygonzkevmfeijoaRaw struct {
	Contract *Polygonzkevmfeijoa // Generic contract binding to access the raw methods on
}

// PolygonzkevmfeijoaCallerRaw is an auto generated low-level read-only Go binding around an Ethereum contract.
type PolygonzkevmfeijoaCallerRaw struct {
	Contract *PolygonzkevmfeijoaCaller // Generic read-only contract binding to access the raw methods on
}

// PolygonzkevmfeijoaTransactorRaw is an auto generated low-level write-only Go binding around an Ethereum contract.
type PolygonzkevmfeijoaTransactorRaw struct {
	Contract *PolygonzkevmfeijoaTransactor // Generic write-only contract binding to access the raw methods on
}

// NewPolygonzkevmfeijoa creates a new instance of Polygonzkevmfeijoa, bound to a specific deployed contract.
func NewPolygonzkevmfeijoa(address common.Address, backend bind.ContractBackend) (*Polygonzkevmfeijoa, error) {
	contract, err := bindPolygonzkevmfeijoa(address, backend, backend, backend)
	if err != nil {
		return nil, err
	}
	return &Polygonzkevmfeijoa{PolygonzkevmfeijoaCaller: PolygonzkevmfeijoaCaller{contract: contract}, PolygonzkevmfeijoaTransactor: PolygonzkevmfeijoaTransactor{contract: contract}, PolygonzkevmfeijoaFilterer: PolygonzkevmfeijoaFilterer{contract: contract}}, nil
}

// NewPolygonzkevmfeijoaCaller creates a new read-only instance of Polygonzkevmfeijoa, bound to a specific deployed contract.
func NewPolygonzkevmfeijoaCaller(address common.Address, caller bind.ContractCaller) (*PolygonzkevmfeijoaCaller, error) {
	contract, err := bindPolygonzkevmfeijoa(address, caller, nil, nil)
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaCaller{contract: contract}, nil
}

// NewPolygonzkevmfeijoaTransactor creates a new write-only instance of Polygonzkevmfeijoa, bound to a specific deployed contract.
func NewPolygonzkevmfeijoaTransactor(address common.Address, transactor bind.ContractTransactor) (*PolygonzkevmfeijoaTransactor, error) {
	contract, err := bindPolygonzkevmfeijoa(address, nil, transactor, nil)
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaTransactor{contract: contract}, nil
}

// NewPolygonzkevmfeijoaFilterer creates a new log filterer instance of Polygonzkevmfeijoa, bound to a specific deployed contract.
func NewPolygonzkevmfeijoaFilterer(address common.Address, filterer bind.ContractFilterer) (*PolygonzkevmfeijoaFilterer, error) {
	contract, err := bindPolygonzkevmfeijoa(address, nil, nil, filterer)
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaFilterer{contract: contract}, nil
}

// bindPolygonzkevmfeijoa binds a generic wrapper to an already deployed contract.
func bindPolygonzkevmfeijoa(address common.Address, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
	parsed, err := PolygonzkevmfeijoaMetaData.GetAbi()
	if err != nil {
		return nil, err
	}
	return bind.NewBoundContract(address, *parsed, caller, transactor, filterer), nil
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _Polygonzkevmfeijoa.Contract.PolygonzkevmfeijoaCaller.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.PolygonzkevmfeijoaTransactor.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.PolygonzkevmfeijoaTransactor.contract.Transact(opts, method, params...)
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _Polygonzkevmfeijoa.Contract.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.contract.Transact(opts, method, params...)
}

// GLOBALEXITROOTMANAGERL2 is a free data retrieval call binding the contract method 0x9e001877.
//
// Solidity: function GLOBAL_EXIT_ROOT_MANAGER_L2() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) GLOBALEXITROOTMANAGERL2(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "GLOBAL_EXIT_ROOT_MANAGER_L2")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// GLOBALEXITROOTMANAGERL2 is a free data retrieval call binding the contract method 0x9e001877.
//
// Solidity: function GLOBAL_EXIT_ROOT_MANAGER_L2() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) GLOBALEXITROOTMANAGERL2() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.GLOBALEXITROOTMANAGERL2(&_Polygonzkevmfeijoa.CallOpts)
}

// GLOBALEXITROOTMANAGERL2 is a free data retrieval call binding the contract method 0x9e001877.
//
// Solidity: function GLOBAL_EXIT_ROOT_MANAGER_L2() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) GLOBALEXITROOTMANAGERL2() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.GLOBALEXITROOTMANAGERL2(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXBRIDGELISTLENLEN is a free data retrieval call binding the contract method 0x11e892d4.
//
// Solidity: function INITIALIZE_TX_BRIDGE_LIST_LEN_LEN() view returns(uint8)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) INITIALIZETXBRIDGELISTLENLEN(opts *bind.CallOpts) (uint8, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "INITIALIZE_TX_BRIDGE_LIST_LEN_LEN")

	if err != nil {
		return *new(uint8), err
	}

	out0 := *abi.ConvertType(out[0], new(uint8)).(*uint8)

	return out0, err

}

// INITIALIZETXBRIDGELISTLENLEN is a free data retrieval call binding the contract method 0x11e892d4.
//
// Solidity: function INITIALIZE_TX_BRIDGE_LIST_LEN_LEN() view returns(uint8)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) INITIALIZETXBRIDGELISTLENLEN() (uint8, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXBRIDGELISTLENLEN(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXBRIDGELISTLENLEN is a free data retrieval call binding the contract method 0x11e892d4.
//
// Solidity: function INITIALIZE_TX_BRIDGE_LIST_LEN_LEN() view returns(uint8)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) INITIALIZETXBRIDGELISTLENLEN() (uint8, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXBRIDGELISTLENLEN(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXBRIDGEPARAMS is a free data retrieval call binding the contract method 0x05835f37.
//
// Solidity: function INITIALIZE_TX_BRIDGE_PARAMS() view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) INITIALIZETXBRIDGEPARAMS(opts *bind.CallOpts) ([]byte, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "INITIALIZE_TX_BRIDGE_PARAMS")

	if err != nil {
		return *new([]byte), err
	}

	out0 := *abi.ConvertType(out[0], new([]byte)).(*[]byte)

	return out0, err

}

// INITIALIZETXBRIDGEPARAMS is a free data retrieval call binding the contract method 0x05835f37.
//
// Solidity: function INITIALIZE_TX_BRIDGE_PARAMS() view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) INITIALIZETXBRIDGEPARAMS() ([]byte, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXBRIDGEPARAMS(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXBRIDGEPARAMS is a free data retrieval call binding the contract method 0x05835f37.
//
// Solidity: function INITIALIZE_TX_BRIDGE_PARAMS() view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) INITIALIZETXBRIDGEPARAMS() ([]byte, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXBRIDGEPARAMS(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESS is a free data retrieval call binding the contract method 0x7a5460c5.
//
// Solidity: function INITIALIZE_TX_BRIDGE_PARAMS_AFTER_BRIDGE_ADDRESS() view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESS(opts *bind.CallOpts) ([]byte, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "INITIALIZE_TX_BRIDGE_PARAMS_AFTER_BRIDGE_ADDRESS")

	if err != nil {
		return *new([]byte), err
	}

	out0 := *abi.ConvertType(out[0], new([]byte)).(*[]byte)

	return out0, err

}

// INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESS is a free data retrieval call binding the contract method 0x7a5460c5.
//
// Solidity: function INITIALIZE_TX_BRIDGE_PARAMS_AFTER_BRIDGE_ADDRESS() view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESS() ([]byte, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESS(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESS is a free data retrieval call binding the contract method 0x7a5460c5.
//
// Solidity: function INITIALIZE_TX_BRIDGE_PARAMS_AFTER_BRIDGE_ADDRESS() view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESS() ([]byte, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESS(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESSEMPTYMETADATA is a free data retrieval call binding the contract method 0x52bdeb6d.
//
// Solidity: function INITIALIZE_TX_BRIDGE_PARAMS_AFTER_BRIDGE_ADDRESS_EMPTY_METADATA() view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESSEMPTYMETADATA(opts *bind.CallOpts) ([]byte, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "INITIALIZE_TX_BRIDGE_PARAMS_AFTER_BRIDGE_ADDRESS_EMPTY_METADATA")

	if err != nil {
		return *new([]byte), err
	}

	out0 := *abi.ConvertType(out[0], new([]byte)).(*[]byte)

	return out0, err

}

// INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESSEMPTYMETADATA is a free data retrieval call binding the contract method 0x52bdeb6d.
//
// Solidity: function INITIALIZE_TX_BRIDGE_PARAMS_AFTER_BRIDGE_ADDRESS_EMPTY_METADATA() view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESSEMPTYMETADATA() ([]byte, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESSEMPTYMETADATA(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESSEMPTYMETADATA is a free data retrieval call binding the contract method 0x52bdeb6d.
//
// Solidity: function INITIALIZE_TX_BRIDGE_PARAMS_AFTER_BRIDGE_ADDRESS_EMPTY_METADATA() view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESSEMPTYMETADATA() ([]byte, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXBRIDGEPARAMSAFTERBRIDGEADDRESSEMPTYMETADATA(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXCONSTANTBYTES is a free data retrieval call binding the contract method 0x03508963.
//
// Solidity: function INITIALIZE_TX_CONSTANT_BYTES() view returns(uint16)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) INITIALIZETXCONSTANTBYTES(opts *bind.CallOpts) (uint16, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "INITIALIZE_TX_CONSTANT_BYTES")

	if err != nil {
		return *new(uint16), err
	}

	out0 := *abi.ConvertType(out[0], new(uint16)).(*uint16)

	return out0, err

}

// INITIALIZETXCONSTANTBYTES is a free data retrieval call binding the contract method 0x03508963.
//
// Solidity: function INITIALIZE_TX_CONSTANT_BYTES() view returns(uint16)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) INITIALIZETXCONSTANTBYTES() (uint16, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXCONSTANTBYTES(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXCONSTANTBYTES is a free data retrieval call binding the contract method 0x03508963.
//
// Solidity: function INITIALIZE_TX_CONSTANT_BYTES() view returns(uint16)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) INITIALIZETXCONSTANTBYTES() (uint16, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXCONSTANTBYTES(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXCONSTANTBYTESEMPTYMETADATA is a free data retrieval call binding the contract method 0x676870d2.
//
// Solidity: function INITIALIZE_TX_CONSTANT_BYTES_EMPTY_METADATA() view returns(uint16)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) INITIALIZETXCONSTANTBYTESEMPTYMETADATA(opts *bind.CallOpts) (uint16, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "INITIALIZE_TX_CONSTANT_BYTES_EMPTY_METADATA")

	if err != nil {
		return *new(uint16), err
	}

	out0 := *abi.ConvertType(out[0], new(uint16)).(*uint16)

	return out0, err

}

// INITIALIZETXCONSTANTBYTESEMPTYMETADATA is a free data retrieval call binding the contract method 0x676870d2.
//
// Solidity: function INITIALIZE_TX_CONSTANT_BYTES_EMPTY_METADATA() view returns(uint16)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) INITIALIZETXCONSTANTBYTESEMPTYMETADATA() (uint16, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXCONSTANTBYTESEMPTYMETADATA(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXCONSTANTBYTESEMPTYMETADATA is a free data retrieval call binding the contract method 0x676870d2.
//
// Solidity: function INITIALIZE_TX_CONSTANT_BYTES_EMPTY_METADATA() view returns(uint16)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) INITIALIZETXCONSTANTBYTESEMPTYMETADATA() (uint16, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXCONSTANTBYTESEMPTYMETADATA(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXDATALENEMPTYMETADATA is a free data retrieval call binding the contract method 0xc7fffd4b.
//
// Solidity: function INITIALIZE_TX_DATA_LEN_EMPTY_METADATA() view returns(uint8)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) INITIALIZETXDATALENEMPTYMETADATA(opts *bind.CallOpts) (uint8, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "INITIALIZE_TX_DATA_LEN_EMPTY_METADATA")

	if err != nil {
		return *new(uint8), err
	}

	out0 := *abi.ConvertType(out[0], new(uint8)).(*uint8)

	return out0, err

}

// INITIALIZETXDATALENEMPTYMETADATA is a free data retrieval call binding the contract method 0xc7fffd4b.
//
// Solidity: function INITIALIZE_TX_DATA_LEN_EMPTY_METADATA() view returns(uint8)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) INITIALIZETXDATALENEMPTYMETADATA() (uint8, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXDATALENEMPTYMETADATA(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXDATALENEMPTYMETADATA is a free data retrieval call binding the contract method 0xc7fffd4b.
//
// Solidity: function INITIALIZE_TX_DATA_LEN_EMPTY_METADATA() view returns(uint8)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) INITIALIZETXDATALENEMPTYMETADATA() (uint8, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXDATALENEMPTYMETADATA(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXEFFECTIVEPERCENTAGE is a free data retrieval call binding the contract method 0x40b5de6c.
//
// Solidity: function INITIALIZE_TX_EFFECTIVE_PERCENTAGE() view returns(bytes1)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) INITIALIZETXEFFECTIVEPERCENTAGE(opts *bind.CallOpts) ([1]byte, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "INITIALIZE_TX_EFFECTIVE_PERCENTAGE")

	if err != nil {
		return *new([1]byte), err
	}

	out0 := *abi.ConvertType(out[0], new([1]byte)).(*[1]byte)

	return out0, err

}

// INITIALIZETXEFFECTIVEPERCENTAGE is a free data retrieval call binding the contract method 0x40b5de6c.
//
// Solidity: function INITIALIZE_TX_EFFECTIVE_PERCENTAGE() view returns(bytes1)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) INITIALIZETXEFFECTIVEPERCENTAGE() ([1]byte, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXEFFECTIVEPERCENTAGE(&_Polygonzkevmfeijoa.CallOpts)
}

// INITIALIZETXEFFECTIVEPERCENTAGE is a free data retrieval call binding the contract method 0x40b5de6c.
//
// Solidity: function INITIALIZE_TX_EFFECTIVE_PERCENTAGE() view returns(bytes1)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) INITIALIZETXEFFECTIVEPERCENTAGE() ([1]byte, error) {
	return _Polygonzkevmfeijoa.Contract.INITIALIZETXEFFECTIVEPERCENTAGE(&_Polygonzkevmfeijoa.CallOpts)
}

// MAXSEQUENCETIMESTAMPFORCED is a free data retrieval call binding the contract method 0x7160c5f7.
//
// Solidity: function MAX_SEQUENCE_TIMESTAMP_FORCED() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) MAXSEQUENCETIMESTAMPFORCED(opts *bind.CallOpts) (uint64, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "MAX_SEQUENCE_TIMESTAMP_FORCED")

	if err != nil {
		return *new(uint64), err
	}

	out0 := *abi.ConvertType(out[0], new(uint64)).(*uint64)

	return out0, err

}

// MAXSEQUENCETIMESTAMPFORCED is a free data retrieval call binding the contract method 0x7160c5f7.
//
// Solidity: function MAX_SEQUENCE_TIMESTAMP_FORCED() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) MAXSEQUENCETIMESTAMPFORCED() (uint64, error) {
	return _Polygonzkevmfeijoa.Contract.MAXSEQUENCETIMESTAMPFORCED(&_Polygonzkevmfeijoa.CallOpts)
}

// MAXSEQUENCETIMESTAMPFORCED is a free data retrieval call binding the contract method 0x7160c5f7.
//
// Solidity: function MAX_SEQUENCE_TIMESTAMP_FORCED() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) MAXSEQUENCETIMESTAMPFORCED() (uint64, error) {
	return _Polygonzkevmfeijoa.Contract.MAXSEQUENCETIMESTAMPFORCED(&_Polygonzkevmfeijoa.CallOpts)
}

// POINTEVALUATIONPRECOMPILEADDRESS is a free data retrieval call binding the contract method 0x3e41062e.
//
// Solidity: function POINT_EVALUATION_PRECOMPILE_ADDRESS() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) POINTEVALUATIONPRECOMPILEADDRESS(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "POINT_EVALUATION_PRECOMPILE_ADDRESS")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// POINTEVALUATIONPRECOMPILEADDRESS is a free data retrieval call binding the contract method 0x3e41062e.
//
// Solidity: function POINT_EVALUATION_PRECOMPILE_ADDRESS() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) POINTEVALUATIONPRECOMPILEADDRESS() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.POINTEVALUATIONPRECOMPILEADDRESS(&_Polygonzkevmfeijoa.CallOpts)
}

// POINTEVALUATIONPRECOMPILEADDRESS is a free data retrieval call binding the contract method 0x3e41062e.
//
// Solidity: function POINT_EVALUATION_PRECOMPILE_ADDRESS() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) POINTEVALUATIONPRECOMPILEADDRESS() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.POINTEVALUATIONPRECOMPILEADDRESS(&_Polygonzkevmfeijoa.CallOpts)
}

// SIGNATUREINITIALIZETXR is a free data retrieval call binding the contract method 0xb0afe154.
//
// Solidity: function SIGNATURE_INITIALIZE_TX_R() view returns(bytes32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) SIGNATUREINITIALIZETXR(opts *bind.CallOpts) ([32]byte, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "SIGNATURE_INITIALIZE_TX_R")

	if err != nil {
		return *new([32]byte), err
	}

	out0 := *abi.ConvertType(out[0], new([32]byte)).(*[32]byte)

	return out0, err

}

// SIGNATUREINITIALIZETXR is a free data retrieval call binding the contract method 0xb0afe154.
//
// Solidity: function SIGNATURE_INITIALIZE_TX_R() view returns(bytes32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) SIGNATUREINITIALIZETXR() ([32]byte, error) {
	return _Polygonzkevmfeijoa.Contract.SIGNATUREINITIALIZETXR(&_Polygonzkevmfeijoa.CallOpts)
}

// SIGNATUREINITIALIZETXR is a free data retrieval call binding the contract method 0xb0afe154.
//
// Solidity: function SIGNATURE_INITIALIZE_TX_R() view returns(bytes32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) SIGNATUREINITIALIZETXR() ([32]byte, error) {
	return _Polygonzkevmfeijoa.Contract.SIGNATUREINITIALIZETXR(&_Polygonzkevmfeijoa.CallOpts)
}

// SIGNATUREINITIALIZETXS is a free data retrieval call binding the contract method 0xd7bc90ff.
//
// Solidity: function SIGNATURE_INITIALIZE_TX_S() view returns(bytes32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) SIGNATUREINITIALIZETXS(opts *bind.CallOpts) ([32]byte, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "SIGNATURE_INITIALIZE_TX_S")

	if err != nil {
		return *new([32]byte), err
	}

	out0 := *abi.ConvertType(out[0], new([32]byte)).(*[32]byte)

	return out0, err

}

// SIGNATUREINITIALIZETXS is a free data retrieval call binding the contract method 0xd7bc90ff.
//
// Solidity: function SIGNATURE_INITIALIZE_TX_S() view returns(bytes32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) SIGNATUREINITIALIZETXS() ([32]byte, error) {
	return _Polygonzkevmfeijoa.Contract.SIGNATUREINITIALIZETXS(&_Polygonzkevmfeijoa.CallOpts)
}

// SIGNATUREINITIALIZETXS is a free data retrieval call binding the contract method 0xd7bc90ff.
//
// Solidity: function SIGNATURE_INITIALIZE_TX_S() view returns(bytes32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) SIGNATUREINITIALIZETXS() ([32]byte, error) {
	return _Polygonzkevmfeijoa.Contract.SIGNATUREINITIALIZETXS(&_Polygonzkevmfeijoa.CallOpts)
}

// SIGNATUREINITIALIZETXV is a free data retrieval call binding the contract method 0xf35dda47.
//
// Solidity: function SIGNATURE_INITIALIZE_TX_V() view returns(uint8)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) SIGNATUREINITIALIZETXV(opts *bind.CallOpts) (uint8, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "SIGNATURE_INITIALIZE_TX_V")

	if err != nil {
		return *new(uint8), err
	}

	out0 := *abi.ConvertType(out[0], new(uint8)).(*uint8)

	return out0, err

}

// SIGNATUREINITIALIZETXV is a free data retrieval call binding the contract method 0xf35dda47.
//
// Solidity: function SIGNATURE_INITIALIZE_TX_V() view returns(uint8)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) SIGNATUREINITIALIZETXV() (uint8, error) {
	return _Polygonzkevmfeijoa.Contract.SIGNATUREINITIALIZETXV(&_Polygonzkevmfeijoa.CallOpts)
}

// SIGNATUREINITIALIZETXV is a free data retrieval call binding the contract method 0xf35dda47.
//
// Solidity: function SIGNATURE_INITIALIZE_TX_V() view returns(uint8)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) SIGNATUREINITIALIZETXV() (uint8, error) {
	return _Polygonzkevmfeijoa.Contract.SIGNATUREINITIALIZETXV(&_Polygonzkevmfeijoa.CallOpts)
}

// TIMESTAMPRANGE is a free data retrieval call binding the contract method 0x42308fab.
//
// Solidity: function TIMESTAMP_RANGE() view returns(uint256)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) TIMESTAMPRANGE(opts *bind.CallOpts) (*big.Int, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "TIMESTAMP_RANGE")

	if err != nil {
		return *new(*big.Int), err
	}

	out0 := *abi.ConvertType(out[0], new(*big.Int)).(**big.Int)

	return out0, err

}

// TIMESTAMPRANGE is a free data retrieval call binding the contract method 0x42308fab.
//
// Solidity: function TIMESTAMP_RANGE() view returns(uint256)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) TIMESTAMPRANGE() (*big.Int, error) {
	return _Polygonzkevmfeijoa.Contract.TIMESTAMPRANGE(&_Polygonzkevmfeijoa.CallOpts)
}

// TIMESTAMPRANGE is a free data retrieval call binding the contract method 0x42308fab.
//
// Solidity: function TIMESTAMP_RANGE() view returns(uint256)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) TIMESTAMPRANGE() (*big.Int, error) {
	return _Polygonzkevmfeijoa.Contract.TIMESTAMPRANGE(&_Polygonzkevmfeijoa.CallOpts)
}

// ZKGASLIMITBATCH is a free data retrieval call binding the contract method 0x838a2503.
//
// Solidity: function ZK_GAS_LIMIT_BATCH() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) ZKGASLIMITBATCH(opts *bind.CallOpts) (uint64, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "ZK_GAS_LIMIT_BATCH")

	if err != nil {
		return *new(uint64), err
	}

	out0 := *abi.ConvertType(out[0], new(uint64)).(*uint64)

	return out0, err

}

// ZKGASLIMITBATCH is a free data retrieval call binding the contract method 0x838a2503.
//
// Solidity: function ZK_GAS_LIMIT_BATCH() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) ZKGASLIMITBATCH() (uint64, error) {
	return _Polygonzkevmfeijoa.Contract.ZKGASLIMITBATCH(&_Polygonzkevmfeijoa.CallOpts)
}

// ZKGASLIMITBATCH is a free data retrieval call binding the contract method 0x838a2503.
//
// Solidity: function ZK_GAS_LIMIT_BATCH() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) ZKGASLIMITBATCH() (uint64, error) {
	return _Polygonzkevmfeijoa.Contract.ZKGASLIMITBATCH(&_Polygonzkevmfeijoa.CallOpts)
}

// Admin is a free data retrieval call binding the contract method 0xf851a440.
//
// Solidity: function admin() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) Admin(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "admin")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// Admin is a free data retrieval call binding the contract method 0xf851a440.
//
// Solidity: function admin() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) Admin() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.Admin(&_Polygonzkevmfeijoa.CallOpts)
}

// Admin is a free data retrieval call binding the contract method 0xf851a440.
//
// Solidity: function admin() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) Admin() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.Admin(&_Polygonzkevmfeijoa.CallOpts)
}

// BridgeAddress is a free data retrieval call binding the contract method 0xa3c573eb.
//
// Solidity: function bridgeAddress() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) BridgeAddress(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "bridgeAddress")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// BridgeAddress is a free data retrieval call binding the contract method 0xa3c573eb.
//
// Solidity: function bridgeAddress() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) BridgeAddress() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.BridgeAddress(&_Polygonzkevmfeijoa.CallOpts)
}

// BridgeAddress is a free data retrieval call binding the contract method 0xa3c573eb.
//
// Solidity: function bridgeAddress() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) BridgeAddress() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.BridgeAddress(&_Polygonzkevmfeijoa.CallOpts)
}

// CalculatePolPerForcedZkGas is a free data retrieval call binding the contract method 0x042b0f06.
//
// Solidity: function calculatePolPerForcedZkGas() view returns(uint256)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) CalculatePolPerForcedZkGas(opts *bind.CallOpts) (*big.Int, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "calculatePolPerForcedZkGas")

	if err != nil {
		return *new(*big.Int), err
	}

	out0 := *abi.ConvertType(out[0], new(*big.Int)).(**big.Int)

	return out0, err

}

// CalculatePolPerForcedZkGas is a free data retrieval call binding the contract method 0x042b0f06.
//
// Solidity: function calculatePolPerForcedZkGas() view returns(uint256)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) CalculatePolPerForcedZkGas() (*big.Int, error) {
	return _Polygonzkevmfeijoa.Contract.CalculatePolPerForcedZkGas(&_Polygonzkevmfeijoa.CallOpts)
}

// CalculatePolPerForcedZkGas is a free data retrieval call binding the contract method 0x042b0f06.
//
// Solidity: function calculatePolPerForcedZkGas() view returns(uint256)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) CalculatePolPerForcedZkGas() (*big.Int, error) {
	return _Polygonzkevmfeijoa.Contract.CalculatePolPerForcedZkGas(&_Polygonzkevmfeijoa.CallOpts)
}

// ForceBlobAddress is a free data retrieval call binding the contract method 0x66e7bb1a.
//
// Solidity: function forceBlobAddress() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) ForceBlobAddress(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "forceBlobAddress")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// ForceBlobAddress is a free data retrieval call binding the contract method 0x66e7bb1a.
//
// Solidity: function forceBlobAddress() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) ForceBlobAddress() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.ForceBlobAddress(&_Polygonzkevmfeijoa.CallOpts)
}

// ForceBlobAddress is a free data retrieval call binding the contract method 0x66e7bb1a.
//
// Solidity: function forceBlobAddress() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) ForceBlobAddress() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.ForceBlobAddress(&_Polygonzkevmfeijoa.CallOpts)
}

// ForceBlobTimeout is a free data retrieval call binding the contract method 0x2c2251db.
//
// Solidity: function forceBlobTimeout() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) ForceBlobTimeout(opts *bind.CallOpts) (uint64, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "forceBlobTimeout")

	if err != nil {
		return *new(uint64), err
	}

	out0 := *abi.ConvertType(out[0], new(uint64)).(*uint64)

	return out0, err

}

// ForceBlobTimeout is a free data retrieval call binding the contract method 0x2c2251db.
//
// Solidity: function forceBlobTimeout() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) ForceBlobTimeout() (uint64, error) {
	return _Polygonzkevmfeijoa.Contract.ForceBlobTimeout(&_Polygonzkevmfeijoa.CallOpts)
}

// ForceBlobTimeout is a free data retrieval call binding the contract method 0x2c2251db.
//
// Solidity: function forceBlobTimeout() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) ForceBlobTimeout() (uint64, error) {
	return _Polygonzkevmfeijoa.Contract.ForceBlobTimeout(&_Polygonzkevmfeijoa.CallOpts)
}

// ForcedBlobs is a free data retrieval call binding the contract method 0x2a6688ee.
//
// Solidity: function forcedBlobs(uint64 ) view returns(bytes32 hashedForcedBlobData, uint64 forcedTimestamp)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) ForcedBlobs(opts *bind.CallOpts, arg0 uint64) (struct {
	HashedForcedBlobData [32]byte
	ForcedTimestamp      uint64
}, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "forcedBlobs", arg0)

	outstruct := new(struct {
		HashedForcedBlobData [32]byte
		ForcedTimestamp      uint64
	})
	if err != nil {
		return *outstruct, err
	}

	outstruct.HashedForcedBlobData = *abi.ConvertType(out[0], new([32]byte)).(*[32]byte)
	outstruct.ForcedTimestamp = *abi.ConvertType(out[1], new(uint64)).(*uint64)

	return *outstruct, err

}

// ForcedBlobs is a free data retrieval call binding the contract method 0x2a6688ee.
//
// Solidity: function forcedBlobs(uint64 ) view returns(bytes32 hashedForcedBlobData, uint64 forcedTimestamp)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) ForcedBlobs(arg0 uint64) (struct {
	HashedForcedBlobData [32]byte
	ForcedTimestamp      uint64
}, error) {
	return _Polygonzkevmfeijoa.Contract.ForcedBlobs(&_Polygonzkevmfeijoa.CallOpts, arg0)
}

// ForcedBlobs is a free data retrieval call binding the contract method 0x2a6688ee.
//
// Solidity: function forcedBlobs(uint64 ) view returns(bytes32 hashedForcedBlobData, uint64 forcedTimestamp)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) ForcedBlobs(arg0 uint64) (struct {
	HashedForcedBlobData [32]byte
	ForcedTimestamp      uint64
}, error) {
	return _Polygonzkevmfeijoa.Contract.ForcedBlobs(&_Polygonzkevmfeijoa.CallOpts, arg0)
}

// GasTokenAddress is a free data retrieval call binding the contract method 0x3c351e10.
//
// Solidity: function gasTokenAddress() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) GasTokenAddress(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "gasTokenAddress")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// GasTokenAddress is a free data retrieval call binding the contract method 0x3c351e10.
//
// Solidity: function gasTokenAddress() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) GasTokenAddress() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.GasTokenAddress(&_Polygonzkevmfeijoa.CallOpts)
}

// GasTokenAddress is a free data retrieval call binding the contract method 0x3c351e10.
//
// Solidity: function gasTokenAddress() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) GasTokenAddress() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.GasTokenAddress(&_Polygonzkevmfeijoa.CallOpts)
}

// GasTokenNetwork is a free data retrieval call binding the contract method 0x3cbc795b.
//
// Solidity: function gasTokenNetwork() view returns(uint32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) GasTokenNetwork(opts *bind.CallOpts) (uint32, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "gasTokenNetwork")

	if err != nil {
		return *new(uint32), err
	}

	out0 := *abi.ConvertType(out[0], new(uint32)).(*uint32)

	return out0, err

}

// GasTokenNetwork is a free data retrieval call binding the contract method 0x3cbc795b.
//
// Solidity: function gasTokenNetwork() view returns(uint32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) GasTokenNetwork() (uint32, error) {
	return _Polygonzkevmfeijoa.Contract.GasTokenNetwork(&_Polygonzkevmfeijoa.CallOpts)
}

// GasTokenNetwork is a free data retrieval call binding the contract method 0x3cbc795b.
//
// Solidity: function gasTokenNetwork() view returns(uint32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) GasTokenNetwork() (uint32, error) {
	return _Polygonzkevmfeijoa.Contract.GasTokenNetwork(&_Polygonzkevmfeijoa.CallOpts)
}

// GenerateInitializeTransaction is a free data retrieval call binding the contract method 0xa652f26c.
//
// Solidity: function generateInitializeTransaction(uint32 networkID, address _gasTokenAddress, uint32 _gasTokenNetwork, bytes _gasTokenMetadata) view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) GenerateInitializeTransaction(opts *bind.CallOpts, networkID uint32, _gasTokenAddress common.Address, _gasTokenNetwork uint32, _gasTokenMetadata []byte) ([]byte, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "generateInitializeTransaction", networkID, _gasTokenAddress, _gasTokenNetwork, _gasTokenMetadata)

	if err != nil {
		return *new([]byte), err
	}

	out0 := *abi.ConvertType(out[0], new([]byte)).(*[]byte)

	return out0, err

}

// GenerateInitializeTransaction is a free data retrieval call binding the contract method 0xa652f26c.
//
// Solidity: function generateInitializeTransaction(uint32 networkID, address _gasTokenAddress, uint32 _gasTokenNetwork, bytes _gasTokenMetadata) view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) GenerateInitializeTransaction(networkID uint32, _gasTokenAddress common.Address, _gasTokenNetwork uint32, _gasTokenMetadata []byte) ([]byte, error) {
	return _Polygonzkevmfeijoa.Contract.GenerateInitializeTransaction(&_Polygonzkevmfeijoa.CallOpts, networkID, _gasTokenAddress, _gasTokenNetwork, _gasTokenMetadata)
}

// GenerateInitializeTransaction is a free data retrieval call binding the contract method 0xa652f26c.
//
// Solidity: function generateInitializeTransaction(uint32 networkID, address _gasTokenAddress, uint32 _gasTokenNetwork, bytes _gasTokenMetadata) view returns(bytes)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) GenerateInitializeTransaction(networkID uint32, _gasTokenAddress common.Address, _gasTokenNetwork uint32, _gasTokenMetadata []byte) ([]byte, error) {
	return _Polygonzkevmfeijoa.Contract.GenerateInitializeTransaction(&_Polygonzkevmfeijoa.CallOpts, networkID, _gasTokenAddress, _gasTokenNetwork, _gasTokenMetadata)
}

// GlobalExitRootManager is a free data retrieval call binding the contract method 0xd02103ca.
//
// Solidity: function globalExitRootManager() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) GlobalExitRootManager(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "globalExitRootManager")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// GlobalExitRootManager is a free data retrieval call binding the contract method 0xd02103ca.
//
// Solidity: function globalExitRootManager() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) GlobalExitRootManager() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.GlobalExitRootManager(&_Polygonzkevmfeijoa.CallOpts)
}

// GlobalExitRootManager is a free data retrieval call binding the contract method 0xd02103ca.
//
// Solidity: function globalExitRootManager() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) GlobalExitRootManager() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.GlobalExitRootManager(&_Polygonzkevmfeijoa.CallOpts)
}

// LastAccInputHash is a free data retrieval call binding the contract method 0x6e05d2cd.
//
// Solidity: function lastAccInputHash() view returns(bytes32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) LastAccInputHash(opts *bind.CallOpts) ([32]byte, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "lastAccInputHash")

	if err != nil {
		return *new([32]byte), err
	}

	out0 := *abi.ConvertType(out[0], new([32]byte)).(*[32]byte)

	return out0, err

}

// LastAccInputHash is a free data retrieval call binding the contract method 0x6e05d2cd.
//
// Solidity: function lastAccInputHash() view returns(bytes32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) LastAccInputHash() ([32]byte, error) {
	return _Polygonzkevmfeijoa.Contract.LastAccInputHash(&_Polygonzkevmfeijoa.CallOpts)
}

// LastAccInputHash is a free data retrieval call binding the contract method 0x6e05d2cd.
//
// Solidity: function lastAccInputHash() view returns(bytes32)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) LastAccInputHash() ([32]byte, error) {
	return _Polygonzkevmfeijoa.Contract.LastAccInputHash(&_Polygonzkevmfeijoa.CallOpts)
}

// LastForceBlob is a free data retrieval call binding the contract method 0x9b0e35a5.
//
// Solidity: function lastForceBlob() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) LastForceBlob(opts *bind.CallOpts) (uint64, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "lastForceBlob")

	if err != nil {
		return *new(uint64), err
	}

	out0 := *abi.ConvertType(out[0], new(uint64)).(*uint64)

	return out0, err

}

// LastForceBlob is a free data retrieval call binding the contract method 0x9b0e35a5.
//
// Solidity: function lastForceBlob() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) LastForceBlob() (uint64, error) {
	return _Polygonzkevmfeijoa.Contract.LastForceBlob(&_Polygonzkevmfeijoa.CallOpts)
}

// LastForceBlob is a free data retrieval call binding the contract method 0x9b0e35a5.
//
// Solidity: function lastForceBlob() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) LastForceBlob() (uint64, error) {
	return _Polygonzkevmfeijoa.Contract.LastForceBlob(&_Polygonzkevmfeijoa.CallOpts)
}

// LastForceBlobSequenced is a free data retrieval call binding the contract method 0xb45bd7f9.
//
// Solidity: function lastForceBlobSequenced() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) LastForceBlobSequenced(opts *bind.CallOpts) (uint64, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "lastForceBlobSequenced")

	if err != nil {
		return *new(uint64), err
	}

	out0 := *abi.ConvertType(out[0], new(uint64)).(*uint64)

	return out0, err

}

// LastForceBlobSequenced is a free data retrieval call binding the contract method 0xb45bd7f9.
//
// Solidity: function lastForceBlobSequenced() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) LastForceBlobSequenced() (uint64, error) {
	return _Polygonzkevmfeijoa.Contract.LastForceBlobSequenced(&_Polygonzkevmfeijoa.CallOpts)
}

// LastForceBlobSequenced is a free data retrieval call binding the contract method 0xb45bd7f9.
//
// Solidity: function lastForceBlobSequenced() view returns(uint64)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) LastForceBlobSequenced() (uint64, error) {
	return _Polygonzkevmfeijoa.Contract.LastForceBlobSequenced(&_Polygonzkevmfeijoa.CallOpts)
}

// NetworkName is a free data retrieval call binding the contract method 0x107bf28c.
//
// Solidity: function networkName() view returns(string)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) NetworkName(opts *bind.CallOpts) (string, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "networkName")

	if err != nil {
		return *new(string), err
	}

	out0 := *abi.ConvertType(out[0], new(string)).(*string)

	return out0, err

}

// NetworkName is a free data retrieval call binding the contract method 0x107bf28c.
//
// Solidity: function networkName() view returns(string)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) NetworkName() (string, error) {
	return _Polygonzkevmfeijoa.Contract.NetworkName(&_Polygonzkevmfeijoa.CallOpts)
}

// NetworkName is a free data retrieval call binding the contract method 0x107bf28c.
//
// Solidity: function networkName() view returns(string)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) NetworkName() (string, error) {
	return _Polygonzkevmfeijoa.Contract.NetworkName(&_Polygonzkevmfeijoa.CallOpts)
}

// PendingAdmin is a free data retrieval call binding the contract method 0x26782247.
//
// Solidity: function pendingAdmin() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) PendingAdmin(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "pendingAdmin")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// PendingAdmin is a free data retrieval call binding the contract method 0x26782247.
//
// Solidity: function pendingAdmin() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) PendingAdmin() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.PendingAdmin(&_Polygonzkevmfeijoa.CallOpts)
}

// PendingAdmin is a free data retrieval call binding the contract method 0x26782247.
//
// Solidity: function pendingAdmin() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) PendingAdmin() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.PendingAdmin(&_Polygonzkevmfeijoa.CallOpts)
}

// Pol is a free data retrieval call binding the contract method 0xe46761c4.
//
// Solidity: function pol() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) Pol(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "pol")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// Pol is a free data retrieval call binding the contract method 0xe46761c4.
//
// Solidity: function pol() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) Pol() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.Pol(&_Polygonzkevmfeijoa.CallOpts)
}

// Pol is a free data retrieval call binding the contract method 0xe46761c4.
//
// Solidity: function pol() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) Pol() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.Pol(&_Polygonzkevmfeijoa.CallOpts)
}

// RollupManager is a free data retrieval call binding the contract method 0x49b7b802.
//
// Solidity: function rollupManager() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) RollupManager(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "rollupManager")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// RollupManager is a free data retrieval call binding the contract method 0x49b7b802.
//
// Solidity: function rollupManager() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) RollupManager() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.RollupManager(&_Polygonzkevmfeijoa.CallOpts)
}

// RollupManager is a free data retrieval call binding the contract method 0x49b7b802.
//
// Solidity: function rollupManager() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) RollupManager() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.RollupManager(&_Polygonzkevmfeijoa.CallOpts)
}

// TrustedSequencer is a free data retrieval call binding the contract method 0xcfa8ed47.
//
// Solidity: function trustedSequencer() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) TrustedSequencer(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "trustedSequencer")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// TrustedSequencer is a free data retrieval call binding the contract method 0xcfa8ed47.
//
// Solidity: function trustedSequencer() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) TrustedSequencer() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.TrustedSequencer(&_Polygonzkevmfeijoa.CallOpts)
}

// TrustedSequencer is a free data retrieval call binding the contract method 0xcfa8ed47.
//
// Solidity: function trustedSequencer() view returns(address)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) TrustedSequencer() (common.Address, error) {
	return _Polygonzkevmfeijoa.Contract.TrustedSequencer(&_Polygonzkevmfeijoa.CallOpts)
}

// TrustedSequencerURL is a free data retrieval call binding the contract method 0x542028d5.
//
// Solidity: function trustedSequencerURL() view returns(string)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCaller) TrustedSequencerURL(opts *bind.CallOpts) (string, error) {
	var out []interface{}
	err := _Polygonzkevmfeijoa.contract.Call(opts, &out, "trustedSequencerURL")

	if err != nil {
		return *new(string), err
	}

	out0 := *abi.ConvertType(out[0], new(string)).(*string)

	return out0, err

}

// TrustedSequencerURL is a free data retrieval call binding the contract method 0x542028d5.
//
// Solidity: function trustedSequencerURL() view returns(string)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) TrustedSequencerURL() (string, error) {
	return _Polygonzkevmfeijoa.Contract.TrustedSequencerURL(&_Polygonzkevmfeijoa.CallOpts)
}

// TrustedSequencerURL is a free data retrieval call binding the contract method 0x542028d5.
//
// Solidity: function trustedSequencerURL() view returns(string)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaCallerSession) TrustedSequencerURL() (string, error) {
	return _Polygonzkevmfeijoa.Contract.TrustedSequencerURL(&_Polygonzkevmfeijoa.CallOpts)
}

// AcceptAdminRole is a paid mutator transaction binding the contract method 0x8c3d7301.
//
// Solidity: function acceptAdminRole() returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) AcceptAdminRole(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "acceptAdminRole")
}

// AcceptAdminRole is a paid mutator transaction binding the contract method 0x8c3d7301.
//
// Solidity: function acceptAdminRole() returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) AcceptAdminRole() (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.AcceptAdminRole(&_Polygonzkevmfeijoa.TransactOpts)
}

// AcceptAdminRole is a paid mutator transaction binding the contract method 0x8c3d7301.
//
// Solidity: function acceptAdminRole() returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) AcceptAdminRole() (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.AcceptAdminRole(&_Polygonzkevmfeijoa.TransactOpts)
}

// ForceBlob is a paid mutator transaction binding the contract method 0xd2a679b7.
//
// Solidity: function forceBlob(bytes blobData, uint256 polAmount) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) ForceBlob(opts *bind.TransactOpts, blobData []byte, polAmount *big.Int) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "forceBlob", blobData, polAmount)
}

// ForceBlob is a paid mutator transaction binding the contract method 0xd2a679b7.
//
// Solidity: function forceBlob(bytes blobData, uint256 polAmount) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) ForceBlob(blobData []byte, polAmount *big.Int) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.ForceBlob(&_Polygonzkevmfeijoa.TransactOpts, blobData, polAmount)
}

// ForceBlob is a paid mutator transaction binding the contract method 0xd2a679b7.
//
// Solidity: function forceBlob(bytes blobData, uint256 polAmount) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) ForceBlob(blobData []byte, polAmount *big.Int) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.ForceBlob(&_Polygonzkevmfeijoa.TransactOpts, blobData, polAmount)
}

// Initialize is a paid mutator transaction binding the contract method 0x71257022.
//
// Solidity: function initialize(address _admin, address sequencer, uint32 networkID, address _gasTokenAddress, string sequencerURL, string _networkName) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) Initialize(opts *bind.TransactOpts, _admin common.Address, sequencer common.Address, networkID uint32, _gasTokenAddress common.Address, sequencerURL string, _networkName string) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "initialize", _admin, sequencer, networkID, _gasTokenAddress, sequencerURL, _networkName)
}

// Initialize is a paid mutator transaction binding the contract method 0x71257022.
//
// Solidity: function initialize(address _admin, address sequencer, uint32 networkID, address _gasTokenAddress, string sequencerURL, string _networkName) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) Initialize(_admin common.Address, sequencer common.Address, networkID uint32, _gasTokenAddress common.Address, sequencerURL string, _networkName string) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.Initialize(&_Polygonzkevmfeijoa.TransactOpts, _admin, sequencer, networkID, _gasTokenAddress, sequencerURL, _networkName)
}

// Initialize is a paid mutator transaction binding the contract method 0x71257022.
//
// Solidity: function initialize(address _admin, address sequencer, uint32 networkID, address _gasTokenAddress, string sequencerURL, string _networkName) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) Initialize(_admin common.Address, sequencer common.Address, networkID uint32, _gasTokenAddress common.Address, sequencerURL string, _networkName string) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.Initialize(&_Polygonzkevmfeijoa.TransactOpts, _admin, sequencer, networkID, _gasTokenAddress, sequencerURL, _networkName)
}

// OnVerifySequences is a paid mutator transaction binding the contract method 0x889cfd7a.
//
// Solidity: function onVerifySequences(uint64 lastVerifiedSequenceNum, bytes32 newStateRoot, address aggregator) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) OnVerifySequences(opts *bind.TransactOpts, lastVerifiedSequenceNum uint64, newStateRoot [32]byte, aggregator common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "onVerifySequences", lastVerifiedSequenceNum, newStateRoot, aggregator)
}

// OnVerifySequences is a paid mutator transaction binding the contract method 0x889cfd7a.
//
// Solidity: function onVerifySequences(uint64 lastVerifiedSequenceNum, bytes32 newStateRoot, address aggregator) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) OnVerifySequences(lastVerifiedSequenceNum uint64, newStateRoot [32]byte, aggregator common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.OnVerifySequences(&_Polygonzkevmfeijoa.TransactOpts, lastVerifiedSequenceNum, newStateRoot, aggregator)
}

// OnVerifySequences is a paid mutator transaction binding the contract method 0x889cfd7a.
//
// Solidity: function onVerifySequences(uint64 lastVerifiedSequenceNum, bytes32 newStateRoot, address aggregator) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) OnVerifySequences(lastVerifiedSequenceNum uint64, newStateRoot [32]byte, aggregator common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.OnVerifySequences(&_Polygonzkevmfeijoa.TransactOpts, lastVerifiedSequenceNum, newStateRoot, aggregator)
}

// SequenceBlobs is a paid mutator transaction binding the contract method 0x38793b4f.
//
// Solidity: function sequenceBlobs((uint8,bytes)[] blobs, address l2Coinbase, bytes32 finalAccInputHash) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) SequenceBlobs(opts *bind.TransactOpts, blobs []PolygonRollupBaseFeijoaBlobData, l2Coinbase common.Address, finalAccInputHash [32]byte) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "sequenceBlobs", blobs, l2Coinbase, finalAccInputHash)
}

// SequenceBlobs is a paid mutator transaction binding the contract method 0x38793b4f.
//
// Solidity: function sequenceBlobs((uint8,bytes)[] blobs, address l2Coinbase, bytes32 finalAccInputHash) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) SequenceBlobs(blobs []PolygonRollupBaseFeijoaBlobData, l2Coinbase common.Address, finalAccInputHash [32]byte) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SequenceBlobs(&_Polygonzkevmfeijoa.TransactOpts, blobs, l2Coinbase, finalAccInputHash)
}

// SequenceBlobs is a paid mutator transaction binding the contract method 0x38793b4f.
//
// Solidity: function sequenceBlobs((uint8,bytes)[] blobs, address l2Coinbase, bytes32 finalAccInputHash) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) SequenceBlobs(blobs []PolygonRollupBaseFeijoaBlobData, l2Coinbase common.Address, finalAccInputHash [32]byte) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SequenceBlobs(&_Polygonzkevmfeijoa.TransactOpts, blobs, l2Coinbase, finalAccInputHash)
}

// SequenceForceBlobs is a paid mutator transaction binding the contract method 0x93932a91.
//
// Solidity: function sequenceForceBlobs((uint8,bytes)[] blobs) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) SequenceForceBlobs(opts *bind.TransactOpts, blobs []PolygonRollupBaseFeijoaBlobData) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "sequenceForceBlobs", blobs)
}

// SequenceForceBlobs is a paid mutator transaction binding the contract method 0x93932a91.
//
// Solidity: function sequenceForceBlobs((uint8,bytes)[] blobs) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) SequenceForceBlobs(blobs []PolygonRollupBaseFeijoaBlobData) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SequenceForceBlobs(&_Polygonzkevmfeijoa.TransactOpts, blobs)
}

// SequenceForceBlobs is a paid mutator transaction binding the contract method 0x93932a91.
//
// Solidity: function sequenceForceBlobs((uint8,bytes)[] blobs) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) SequenceForceBlobs(blobs []PolygonRollupBaseFeijoaBlobData) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SequenceForceBlobs(&_Polygonzkevmfeijoa.TransactOpts, blobs)
}

// SetForceBlobAddress is a paid mutator transaction binding the contract method 0x4bd41065.
//
// Solidity: function setForceBlobAddress(address newForceBlobAddress) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) SetForceBlobAddress(opts *bind.TransactOpts, newForceBlobAddress common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "setForceBlobAddress", newForceBlobAddress)
}

// SetForceBlobAddress is a paid mutator transaction binding the contract method 0x4bd41065.
//
// Solidity: function setForceBlobAddress(address newForceBlobAddress) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) SetForceBlobAddress(newForceBlobAddress common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SetForceBlobAddress(&_Polygonzkevmfeijoa.TransactOpts, newForceBlobAddress)
}

// SetForceBlobAddress is a paid mutator transaction binding the contract method 0x4bd41065.
//
// Solidity: function setForceBlobAddress(address newForceBlobAddress) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) SetForceBlobAddress(newForceBlobAddress common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SetForceBlobAddress(&_Polygonzkevmfeijoa.TransactOpts, newForceBlobAddress)
}

// SetForceBlobTimeout is a paid mutator transaction binding the contract method 0x730c8e21.
//
// Solidity: function setForceBlobTimeout(uint64 newforceBlobTimeout) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) SetForceBlobTimeout(opts *bind.TransactOpts, newforceBlobTimeout uint64) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "setForceBlobTimeout", newforceBlobTimeout)
}

// SetForceBlobTimeout is a paid mutator transaction binding the contract method 0x730c8e21.
//
// Solidity: function setForceBlobTimeout(uint64 newforceBlobTimeout) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) SetForceBlobTimeout(newforceBlobTimeout uint64) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SetForceBlobTimeout(&_Polygonzkevmfeijoa.TransactOpts, newforceBlobTimeout)
}

// SetForceBlobTimeout is a paid mutator transaction binding the contract method 0x730c8e21.
//
// Solidity: function setForceBlobTimeout(uint64 newforceBlobTimeout) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) SetForceBlobTimeout(newforceBlobTimeout uint64) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SetForceBlobTimeout(&_Polygonzkevmfeijoa.TransactOpts, newforceBlobTimeout)
}

// SetNetworkName is a paid mutator transaction binding the contract method 0xc0cad302.
//
// Solidity: function setNetworkName(string newNetworkName) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) SetNetworkName(opts *bind.TransactOpts, newNetworkName string) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "setNetworkName", newNetworkName)
}

// SetNetworkName is a paid mutator transaction binding the contract method 0xc0cad302.
//
// Solidity: function setNetworkName(string newNetworkName) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) SetNetworkName(newNetworkName string) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SetNetworkName(&_Polygonzkevmfeijoa.TransactOpts, newNetworkName)
}

// SetNetworkName is a paid mutator transaction binding the contract method 0xc0cad302.
//
// Solidity: function setNetworkName(string newNetworkName) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) SetNetworkName(newNetworkName string) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SetNetworkName(&_Polygonzkevmfeijoa.TransactOpts, newNetworkName)
}

// SetTrustedSequencer is a paid mutator transaction binding the contract method 0x6ff512cc.
//
// Solidity: function setTrustedSequencer(address newTrustedSequencer) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) SetTrustedSequencer(opts *bind.TransactOpts, newTrustedSequencer common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "setTrustedSequencer", newTrustedSequencer)
}

// SetTrustedSequencer is a paid mutator transaction binding the contract method 0x6ff512cc.
//
// Solidity: function setTrustedSequencer(address newTrustedSequencer) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) SetTrustedSequencer(newTrustedSequencer common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SetTrustedSequencer(&_Polygonzkevmfeijoa.TransactOpts, newTrustedSequencer)
}

// SetTrustedSequencer is a paid mutator transaction binding the contract method 0x6ff512cc.
//
// Solidity: function setTrustedSequencer(address newTrustedSequencer) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) SetTrustedSequencer(newTrustedSequencer common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SetTrustedSequencer(&_Polygonzkevmfeijoa.TransactOpts, newTrustedSequencer)
}

// SetTrustedSequencerURL is a paid mutator transaction binding the contract method 0xc89e42df.
//
// Solidity: function setTrustedSequencerURL(string newTrustedSequencerURL) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) SetTrustedSequencerURL(opts *bind.TransactOpts, newTrustedSequencerURL string) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "setTrustedSequencerURL", newTrustedSequencerURL)
}

// SetTrustedSequencerURL is a paid mutator transaction binding the contract method 0xc89e42df.
//
// Solidity: function setTrustedSequencerURL(string newTrustedSequencerURL) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) SetTrustedSequencerURL(newTrustedSequencerURL string) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SetTrustedSequencerURL(&_Polygonzkevmfeijoa.TransactOpts, newTrustedSequencerURL)
}

// SetTrustedSequencerURL is a paid mutator transaction binding the contract method 0xc89e42df.
//
// Solidity: function setTrustedSequencerURL(string newTrustedSequencerURL) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) SetTrustedSequencerURL(newTrustedSequencerURL string) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.SetTrustedSequencerURL(&_Polygonzkevmfeijoa.TransactOpts, newTrustedSequencerURL)
}

// TransferAdminRole is a paid mutator transaction binding the contract method 0xada8f919.
//
// Solidity: function transferAdminRole(address newPendingAdmin) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactor) TransferAdminRole(opts *bind.TransactOpts, newPendingAdmin common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.contract.Transact(opts, "transferAdminRole", newPendingAdmin)
}

// TransferAdminRole is a paid mutator transaction binding the contract method 0xada8f919.
//
// Solidity: function transferAdminRole(address newPendingAdmin) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaSession) TransferAdminRole(newPendingAdmin common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.TransferAdminRole(&_Polygonzkevmfeijoa.TransactOpts, newPendingAdmin)
}

// TransferAdminRole is a paid mutator transaction binding the contract method 0xada8f919.
//
// Solidity: function transferAdminRole(address newPendingAdmin) returns()
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaTransactorSession) TransferAdminRole(newPendingAdmin common.Address) (*types.Transaction, error) {
	return _Polygonzkevmfeijoa.Contract.TransferAdminRole(&_Polygonzkevmfeijoa.TransactOpts, newPendingAdmin)
}

// PolygonzkevmfeijoaAcceptAdminRoleIterator is returned from FilterAcceptAdminRole and is used to iterate over the raw logs and unpacked data for AcceptAdminRole events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaAcceptAdminRoleIterator struct {
	Event *PolygonzkevmfeijoaAcceptAdminRole // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaAcceptAdminRoleIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaAcceptAdminRole)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaAcceptAdminRole)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaAcceptAdminRoleIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaAcceptAdminRoleIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaAcceptAdminRole represents a AcceptAdminRole event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaAcceptAdminRole struct {
	NewAdmin common.Address
	Raw      types.Log // Blockchain specific contextual infos
}

// FilterAcceptAdminRole is a free log retrieval operation binding the contract event 0x056dc487bbf0795d0bbb1b4f0af523a855503cff740bfb4d5475f7a90c091e8e.
//
// Solidity: event AcceptAdminRole(address newAdmin)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterAcceptAdminRole(opts *bind.FilterOpts) (*PolygonzkevmfeijoaAcceptAdminRoleIterator, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "AcceptAdminRole")
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaAcceptAdminRoleIterator{contract: _Polygonzkevmfeijoa.contract, event: "AcceptAdminRole", logs: logs, sub: sub}, nil
}

// WatchAcceptAdminRole is a free log subscription operation binding the contract event 0x056dc487bbf0795d0bbb1b4f0af523a855503cff740bfb4d5475f7a90c091e8e.
//
// Solidity: event AcceptAdminRole(address newAdmin)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchAcceptAdminRole(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaAcceptAdminRole) (event.Subscription, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "AcceptAdminRole")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaAcceptAdminRole)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "AcceptAdminRole", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseAcceptAdminRole is a log parse operation binding the contract event 0x056dc487bbf0795d0bbb1b4f0af523a855503cff740bfb4d5475f7a90c091e8e.
//
// Solidity: event AcceptAdminRole(address newAdmin)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseAcceptAdminRole(log types.Log) (*PolygonzkevmfeijoaAcceptAdminRole, error) {
	event := new(PolygonzkevmfeijoaAcceptAdminRole)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "AcceptAdminRole", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaForceBlobIterator is returned from FilterForceBlob and is used to iterate over the raw logs and unpacked data for ForceBlob events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaForceBlobIterator struct {
	Event *PolygonzkevmfeijoaForceBlob // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaForceBlobIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaForceBlob)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaForceBlob)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaForceBlobIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaForceBlobIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaForceBlob represents a ForceBlob event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaForceBlob struct {
	ForceBlobNum       uint64
	LastGlobalExitRoot [32]byte
	Sequencer          common.Address
	ZkGasLimit         uint64
	Transactions       []byte
	Raw                types.Log // Blockchain specific contextual infos
}

// FilterForceBlob is a free log retrieval operation binding the contract event 0xb18d758550a6ed34847584be90f0a34b261d8b65bb790891103d5e255aced8b2.
//
// Solidity: event ForceBlob(uint64 indexed forceBlobNum, bytes32 lastGlobalExitRoot, address sequencer, uint64 zkGasLimit, bytes transactions)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterForceBlob(opts *bind.FilterOpts, forceBlobNum []uint64) (*PolygonzkevmfeijoaForceBlobIterator, error) {

	var forceBlobNumRule []interface{}
	for _, forceBlobNumItem := range forceBlobNum {
		forceBlobNumRule = append(forceBlobNumRule, forceBlobNumItem)
	}

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "ForceBlob", forceBlobNumRule)
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaForceBlobIterator{contract: _Polygonzkevmfeijoa.contract, event: "ForceBlob", logs: logs, sub: sub}, nil
}

// WatchForceBlob is a free log subscription operation binding the contract event 0xb18d758550a6ed34847584be90f0a34b261d8b65bb790891103d5e255aced8b2.
//
// Solidity: event ForceBlob(uint64 indexed forceBlobNum, bytes32 lastGlobalExitRoot, address sequencer, uint64 zkGasLimit, bytes transactions)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchForceBlob(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaForceBlob, forceBlobNum []uint64) (event.Subscription, error) {

	var forceBlobNumRule []interface{}
	for _, forceBlobNumItem := range forceBlobNum {
		forceBlobNumRule = append(forceBlobNumRule, forceBlobNumItem)
	}

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "ForceBlob", forceBlobNumRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaForceBlob)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "ForceBlob", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseForceBlob is a log parse operation binding the contract event 0xb18d758550a6ed34847584be90f0a34b261d8b65bb790891103d5e255aced8b2.
//
// Solidity: event ForceBlob(uint64 indexed forceBlobNum, bytes32 lastGlobalExitRoot, address sequencer, uint64 zkGasLimit, bytes transactions)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseForceBlob(log types.Log) (*PolygonzkevmfeijoaForceBlob, error) {
	event := new(PolygonzkevmfeijoaForceBlob)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "ForceBlob", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaInitialSequenceBlobsIterator is returned from FilterInitialSequenceBlobs and is used to iterate over the raw logs and unpacked data for InitialSequenceBlobs events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaInitialSequenceBlobsIterator struct {
	Event *PolygonzkevmfeijoaInitialSequenceBlobs // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaInitialSequenceBlobsIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaInitialSequenceBlobs)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaInitialSequenceBlobs)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaInitialSequenceBlobsIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaInitialSequenceBlobsIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaInitialSequenceBlobs represents a InitialSequenceBlobs event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaInitialSequenceBlobs struct {
	Transactions       []byte
	LastGlobalExitRoot [32]byte
	Sequencer          common.Address
	Raw                types.Log // Blockchain specific contextual infos
}

// FilterInitialSequenceBlobs is a free log retrieval operation binding the contract event 0xfa56300f6f91d53e1c1283e56307c169d72b14a75380df3ecbb5b31b498d3d1e.
//
// Solidity: event InitialSequenceBlobs(bytes transactions, bytes32 lastGlobalExitRoot, address sequencer)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterInitialSequenceBlobs(opts *bind.FilterOpts) (*PolygonzkevmfeijoaInitialSequenceBlobsIterator, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "InitialSequenceBlobs")
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaInitialSequenceBlobsIterator{contract: _Polygonzkevmfeijoa.contract, event: "InitialSequenceBlobs", logs: logs, sub: sub}, nil
}

// WatchInitialSequenceBlobs is a free log subscription operation binding the contract event 0xfa56300f6f91d53e1c1283e56307c169d72b14a75380df3ecbb5b31b498d3d1e.
//
// Solidity: event InitialSequenceBlobs(bytes transactions, bytes32 lastGlobalExitRoot, address sequencer)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchInitialSequenceBlobs(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaInitialSequenceBlobs) (event.Subscription, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "InitialSequenceBlobs")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaInitialSequenceBlobs)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "InitialSequenceBlobs", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseInitialSequenceBlobs is a log parse operation binding the contract event 0xfa56300f6f91d53e1c1283e56307c169d72b14a75380df3ecbb5b31b498d3d1e.
//
// Solidity: event InitialSequenceBlobs(bytes transactions, bytes32 lastGlobalExitRoot, address sequencer)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseInitialSequenceBlobs(log types.Log) (*PolygonzkevmfeijoaInitialSequenceBlobs, error) {
	event := new(PolygonzkevmfeijoaInitialSequenceBlobs)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "InitialSequenceBlobs", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaInitializedIterator is returned from FilterInitialized and is used to iterate over the raw logs and unpacked data for Initialized events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaInitializedIterator struct {
	Event *PolygonzkevmfeijoaInitialized // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaInitializedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaInitialized)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaInitialized)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaInitializedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaInitializedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaInitialized represents a Initialized event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaInitialized struct {
	Version uint8
	Raw     types.Log // Blockchain specific contextual infos
}

// FilterInitialized is a free log retrieval operation binding the contract event 0x7f26b83ff96e1f2b6a682f133852f6798a09c465da95921460cefb3847402498.
//
// Solidity: event Initialized(uint8 version)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterInitialized(opts *bind.FilterOpts) (*PolygonzkevmfeijoaInitializedIterator, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "Initialized")
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaInitializedIterator{contract: _Polygonzkevmfeijoa.contract, event: "Initialized", logs: logs, sub: sub}, nil
}

// WatchInitialized is a free log subscription operation binding the contract event 0x7f26b83ff96e1f2b6a682f133852f6798a09c465da95921460cefb3847402498.
//
// Solidity: event Initialized(uint8 version)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchInitialized(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaInitialized) (event.Subscription, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "Initialized")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaInitialized)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "Initialized", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseInitialized is a log parse operation binding the contract event 0x7f26b83ff96e1f2b6a682f133852f6798a09c465da95921460cefb3847402498.
//
// Solidity: event Initialized(uint8 version)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseInitialized(log types.Log) (*PolygonzkevmfeijoaInitialized, error) {
	event := new(PolygonzkevmfeijoaInitialized)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "Initialized", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaSequenceBlobsIterator is returned from FilterSequenceBlobs and is used to iterate over the raw logs and unpacked data for SequenceBlobs events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSequenceBlobsIterator struct {
	Event *PolygonzkevmfeijoaSequenceBlobs // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaSequenceBlobsIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaSequenceBlobs)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaSequenceBlobs)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaSequenceBlobsIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaSequenceBlobsIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaSequenceBlobs represents a SequenceBlobs event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSequenceBlobs struct {
	LastBlobSequenced uint64
	Raw               types.Log // Blockchain specific contextual infos
}

// FilterSequenceBlobs is a free log retrieval operation binding the contract event 0x470f4ca4b003755c839b80ab00c3efbeb69d6eafec00e1a3677482933ec1fd0c.
//
// Solidity: event SequenceBlobs(uint64 indexed lastBlobSequenced)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterSequenceBlobs(opts *bind.FilterOpts, lastBlobSequenced []uint64) (*PolygonzkevmfeijoaSequenceBlobsIterator, error) {

	var lastBlobSequencedRule []interface{}
	for _, lastBlobSequencedItem := range lastBlobSequenced {
		lastBlobSequencedRule = append(lastBlobSequencedRule, lastBlobSequencedItem)
	}

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "SequenceBlobs", lastBlobSequencedRule)
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaSequenceBlobsIterator{contract: _Polygonzkevmfeijoa.contract, event: "SequenceBlobs", logs: logs, sub: sub}, nil
}

// WatchSequenceBlobs is a free log subscription operation binding the contract event 0x470f4ca4b003755c839b80ab00c3efbeb69d6eafec00e1a3677482933ec1fd0c.
//
// Solidity: event SequenceBlobs(uint64 indexed lastBlobSequenced)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchSequenceBlobs(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaSequenceBlobs, lastBlobSequenced []uint64) (event.Subscription, error) {

	var lastBlobSequencedRule []interface{}
	for _, lastBlobSequencedItem := range lastBlobSequenced {
		lastBlobSequencedRule = append(lastBlobSequencedRule, lastBlobSequencedItem)
	}

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "SequenceBlobs", lastBlobSequencedRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaSequenceBlobs)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SequenceBlobs", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseSequenceBlobs is a log parse operation binding the contract event 0x470f4ca4b003755c839b80ab00c3efbeb69d6eafec00e1a3677482933ec1fd0c.
//
// Solidity: event SequenceBlobs(uint64 indexed lastBlobSequenced)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseSequenceBlobs(log types.Log) (*PolygonzkevmfeijoaSequenceBlobs, error) {
	event := new(PolygonzkevmfeijoaSequenceBlobs)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SequenceBlobs", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaSequenceForceBlobsIterator is returned from FilterSequenceForceBlobs and is used to iterate over the raw logs and unpacked data for SequenceForceBlobs events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSequenceForceBlobsIterator struct {
	Event *PolygonzkevmfeijoaSequenceForceBlobs // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaSequenceForceBlobsIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaSequenceForceBlobs)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaSequenceForceBlobs)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaSequenceForceBlobsIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaSequenceForceBlobsIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaSequenceForceBlobs represents a SequenceForceBlobs event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSequenceForceBlobs struct {
	NumBlob uint64
	Raw     types.Log // Blockchain specific contextual infos
}

// FilterSequenceForceBlobs is a free log retrieval operation binding the contract event 0x049b259b0b684f32f1d8b43d76cf6cb3c674b94697bda3290f6ec63252cfe892.
//
// Solidity: event SequenceForceBlobs(uint64 indexed numBlob)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterSequenceForceBlobs(opts *bind.FilterOpts, numBlob []uint64) (*PolygonzkevmfeijoaSequenceForceBlobsIterator, error) {

	var numBlobRule []interface{}
	for _, numBlobItem := range numBlob {
		numBlobRule = append(numBlobRule, numBlobItem)
	}

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "SequenceForceBlobs", numBlobRule)
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaSequenceForceBlobsIterator{contract: _Polygonzkevmfeijoa.contract, event: "SequenceForceBlobs", logs: logs, sub: sub}, nil
}

// WatchSequenceForceBlobs is a free log subscription operation binding the contract event 0x049b259b0b684f32f1d8b43d76cf6cb3c674b94697bda3290f6ec63252cfe892.
//
// Solidity: event SequenceForceBlobs(uint64 indexed numBlob)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchSequenceForceBlobs(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaSequenceForceBlobs, numBlob []uint64) (event.Subscription, error) {

	var numBlobRule []interface{}
	for _, numBlobItem := range numBlob {
		numBlobRule = append(numBlobRule, numBlobItem)
	}

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "SequenceForceBlobs", numBlobRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaSequenceForceBlobs)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SequenceForceBlobs", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseSequenceForceBlobs is a log parse operation binding the contract event 0x049b259b0b684f32f1d8b43d76cf6cb3c674b94697bda3290f6ec63252cfe892.
//
// Solidity: event SequenceForceBlobs(uint64 indexed numBlob)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseSequenceForceBlobs(log types.Log) (*PolygonzkevmfeijoaSequenceForceBlobs, error) {
	event := new(PolygonzkevmfeijoaSequenceForceBlobs)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SequenceForceBlobs", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaSetForceBlobAddressIterator is returned from FilterSetForceBlobAddress and is used to iterate over the raw logs and unpacked data for SetForceBlobAddress events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSetForceBlobAddressIterator struct {
	Event *PolygonzkevmfeijoaSetForceBlobAddress // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaSetForceBlobAddressIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaSetForceBlobAddress)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaSetForceBlobAddress)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaSetForceBlobAddressIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaSetForceBlobAddressIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaSetForceBlobAddress represents a SetForceBlobAddress event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSetForceBlobAddress struct {
	NewForceBlobAddress common.Address
	Raw                 types.Log // Blockchain specific contextual infos
}

// FilterSetForceBlobAddress is a free log retrieval operation binding the contract event 0x2261b2af55eeb3b995b5e300659fa8e59827ff8fc99ff3a5baf5af0835aab9dd.
//
// Solidity: event SetForceBlobAddress(address newForceBlobAddress)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterSetForceBlobAddress(opts *bind.FilterOpts) (*PolygonzkevmfeijoaSetForceBlobAddressIterator, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "SetForceBlobAddress")
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaSetForceBlobAddressIterator{contract: _Polygonzkevmfeijoa.contract, event: "SetForceBlobAddress", logs: logs, sub: sub}, nil
}

// WatchSetForceBlobAddress is a free log subscription operation binding the contract event 0x2261b2af55eeb3b995b5e300659fa8e59827ff8fc99ff3a5baf5af0835aab9dd.
//
// Solidity: event SetForceBlobAddress(address newForceBlobAddress)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchSetForceBlobAddress(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaSetForceBlobAddress) (event.Subscription, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "SetForceBlobAddress")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaSetForceBlobAddress)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SetForceBlobAddress", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseSetForceBlobAddress is a log parse operation binding the contract event 0x2261b2af55eeb3b995b5e300659fa8e59827ff8fc99ff3a5baf5af0835aab9dd.
//
// Solidity: event SetForceBlobAddress(address newForceBlobAddress)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseSetForceBlobAddress(log types.Log) (*PolygonzkevmfeijoaSetForceBlobAddress, error) {
	event := new(PolygonzkevmfeijoaSetForceBlobAddress)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SetForceBlobAddress", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaSetForceBlobTimeoutIterator is returned from FilterSetForceBlobTimeout and is used to iterate over the raw logs and unpacked data for SetForceBlobTimeout events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSetForceBlobTimeoutIterator struct {
	Event *PolygonzkevmfeijoaSetForceBlobTimeout // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaSetForceBlobTimeoutIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaSetForceBlobTimeout)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaSetForceBlobTimeout)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaSetForceBlobTimeoutIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaSetForceBlobTimeoutIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaSetForceBlobTimeout represents a SetForceBlobTimeout event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSetForceBlobTimeout struct {
	NewforceBlobTimeout uint64
	Raw                 types.Log // Blockchain specific contextual infos
}

// FilterSetForceBlobTimeout is a free log retrieval operation binding the contract event 0xa6db492cb43063288b0b5d7c271f8df34607c41fc9347c0664e1ce325cc728e8.
//
// Solidity: event SetForceBlobTimeout(uint64 newforceBlobTimeout)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterSetForceBlobTimeout(opts *bind.FilterOpts) (*PolygonzkevmfeijoaSetForceBlobTimeoutIterator, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "SetForceBlobTimeout")
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaSetForceBlobTimeoutIterator{contract: _Polygonzkevmfeijoa.contract, event: "SetForceBlobTimeout", logs: logs, sub: sub}, nil
}

// WatchSetForceBlobTimeout is a free log subscription operation binding the contract event 0xa6db492cb43063288b0b5d7c271f8df34607c41fc9347c0664e1ce325cc728e8.
//
// Solidity: event SetForceBlobTimeout(uint64 newforceBlobTimeout)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchSetForceBlobTimeout(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaSetForceBlobTimeout) (event.Subscription, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "SetForceBlobTimeout")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaSetForceBlobTimeout)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SetForceBlobTimeout", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseSetForceBlobTimeout is a log parse operation binding the contract event 0xa6db492cb43063288b0b5d7c271f8df34607c41fc9347c0664e1ce325cc728e8.
//
// Solidity: event SetForceBlobTimeout(uint64 newforceBlobTimeout)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseSetForceBlobTimeout(log types.Log) (*PolygonzkevmfeijoaSetForceBlobTimeout, error) {
	event := new(PolygonzkevmfeijoaSetForceBlobTimeout)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SetForceBlobTimeout", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaSetNetworkNameIterator is returned from FilterSetNetworkName and is used to iterate over the raw logs and unpacked data for SetNetworkName events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSetNetworkNameIterator struct {
	Event *PolygonzkevmfeijoaSetNetworkName // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaSetNetworkNameIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaSetNetworkName)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaSetNetworkName)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaSetNetworkNameIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaSetNetworkNameIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaSetNetworkName represents a SetNetworkName event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSetNetworkName struct {
	NewNetworkName string
	Raw            types.Log // Blockchain specific contextual infos
}

// FilterSetNetworkName is a free log retrieval operation binding the contract event 0xcc3b37f0de47ea5ce245c3502f0d4e414c34664023b8463db2fe451fee5e6992.
//
// Solidity: event SetNetworkName(string newNetworkName)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterSetNetworkName(opts *bind.FilterOpts) (*PolygonzkevmfeijoaSetNetworkNameIterator, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "SetNetworkName")
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaSetNetworkNameIterator{contract: _Polygonzkevmfeijoa.contract, event: "SetNetworkName", logs: logs, sub: sub}, nil
}

// WatchSetNetworkName is a free log subscription operation binding the contract event 0xcc3b37f0de47ea5ce245c3502f0d4e414c34664023b8463db2fe451fee5e6992.
//
// Solidity: event SetNetworkName(string newNetworkName)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchSetNetworkName(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaSetNetworkName) (event.Subscription, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "SetNetworkName")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaSetNetworkName)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SetNetworkName", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseSetNetworkName is a log parse operation binding the contract event 0xcc3b37f0de47ea5ce245c3502f0d4e414c34664023b8463db2fe451fee5e6992.
//
// Solidity: event SetNetworkName(string newNetworkName)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseSetNetworkName(log types.Log) (*PolygonzkevmfeijoaSetNetworkName, error) {
	event := new(PolygonzkevmfeijoaSetNetworkName)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SetNetworkName", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaSetTrustedSequencerIterator is returned from FilterSetTrustedSequencer and is used to iterate over the raw logs and unpacked data for SetTrustedSequencer events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSetTrustedSequencerIterator struct {
	Event *PolygonzkevmfeijoaSetTrustedSequencer // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaSetTrustedSequencerIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaSetTrustedSequencer)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaSetTrustedSequencer)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaSetTrustedSequencerIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaSetTrustedSequencerIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaSetTrustedSequencer represents a SetTrustedSequencer event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSetTrustedSequencer struct {
	NewTrustedSequencer common.Address
	Raw                 types.Log // Blockchain specific contextual infos
}

// FilterSetTrustedSequencer is a free log retrieval operation binding the contract event 0xf54144f9611984021529f814a1cb6a41e22c58351510a0d9f7e822618abb9cc0.
//
// Solidity: event SetTrustedSequencer(address newTrustedSequencer)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterSetTrustedSequencer(opts *bind.FilterOpts) (*PolygonzkevmfeijoaSetTrustedSequencerIterator, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "SetTrustedSequencer")
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaSetTrustedSequencerIterator{contract: _Polygonzkevmfeijoa.contract, event: "SetTrustedSequencer", logs: logs, sub: sub}, nil
}

// WatchSetTrustedSequencer is a free log subscription operation binding the contract event 0xf54144f9611984021529f814a1cb6a41e22c58351510a0d9f7e822618abb9cc0.
//
// Solidity: event SetTrustedSequencer(address newTrustedSequencer)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchSetTrustedSequencer(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaSetTrustedSequencer) (event.Subscription, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "SetTrustedSequencer")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaSetTrustedSequencer)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SetTrustedSequencer", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseSetTrustedSequencer is a log parse operation binding the contract event 0xf54144f9611984021529f814a1cb6a41e22c58351510a0d9f7e822618abb9cc0.
//
// Solidity: event SetTrustedSequencer(address newTrustedSequencer)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseSetTrustedSequencer(log types.Log) (*PolygonzkevmfeijoaSetTrustedSequencer, error) {
	event := new(PolygonzkevmfeijoaSetTrustedSequencer)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SetTrustedSequencer", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaSetTrustedSequencerURLIterator is returned from FilterSetTrustedSequencerURL and is used to iterate over the raw logs and unpacked data for SetTrustedSequencerURL events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSetTrustedSequencerURLIterator struct {
	Event *PolygonzkevmfeijoaSetTrustedSequencerURL // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaSetTrustedSequencerURLIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaSetTrustedSequencerURL)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaSetTrustedSequencerURL)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaSetTrustedSequencerURLIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaSetTrustedSequencerURLIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaSetTrustedSequencerURL represents a SetTrustedSequencerURL event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaSetTrustedSequencerURL struct {
	NewTrustedSequencerURL string
	Raw                    types.Log // Blockchain specific contextual infos
}

// FilterSetTrustedSequencerURL is a free log retrieval operation binding the contract event 0x6b8f723a4c7a5335cafae8a598a0aa0301be1387c037dccc085b62add6448b20.
//
// Solidity: event SetTrustedSequencerURL(string newTrustedSequencerURL)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterSetTrustedSequencerURL(opts *bind.FilterOpts) (*PolygonzkevmfeijoaSetTrustedSequencerURLIterator, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "SetTrustedSequencerURL")
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaSetTrustedSequencerURLIterator{contract: _Polygonzkevmfeijoa.contract, event: "SetTrustedSequencerURL", logs: logs, sub: sub}, nil
}

// WatchSetTrustedSequencerURL is a free log subscription operation binding the contract event 0x6b8f723a4c7a5335cafae8a598a0aa0301be1387c037dccc085b62add6448b20.
//
// Solidity: event SetTrustedSequencerURL(string newTrustedSequencerURL)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchSetTrustedSequencerURL(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaSetTrustedSequencerURL) (event.Subscription, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "SetTrustedSequencerURL")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaSetTrustedSequencerURL)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SetTrustedSequencerURL", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseSetTrustedSequencerURL is a log parse operation binding the contract event 0x6b8f723a4c7a5335cafae8a598a0aa0301be1387c037dccc085b62add6448b20.
//
// Solidity: event SetTrustedSequencerURL(string newTrustedSequencerURL)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseSetTrustedSequencerURL(log types.Log) (*PolygonzkevmfeijoaSetTrustedSequencerURL, error) {
	event := new(PolygonzkevmfeijoaSetTrustedSequencerURL)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "SetTrustedSequencerURL", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaTransferAdminRoleIterator is returned from FilterTransferAdminRole and is used to iterate over the raw logs and unpacked data for TransferAdminRole events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaTransferAdminRoleIterator struct {
	Event *PolygonzkevmfeijoaTransferAdminRole // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaTransferAdminRoleIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaTransferAdminRole)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaTransferAdminRole)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaTransferAdminRoleIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaTransferAdminRoleIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaTransferAdminRole represents a TransferAdminRole event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaTransferAdminRole struct {
	NewPendingAdmin common.Address
	Raw             types.Log // Blockchain specific contextual infos
}

// FilterTransferAdminRole is a free log retrieval operation binding the contract event 0xa5b56b7906fd0a20e3f35120dd8343db1e12e037a6c90111c7e42885e82a1ce6.
//
// Solidity: event TransferAdminRole(address newPendingAdmin)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterTransferAdminRole(opts *bind.FilterOpts) (*PolygonzkevmfeijoaTransferAdminRoleIterator, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "TransferAdminRole")
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaTransferAdminRoleIterator{contract: _Polygonzkevmfeijoa.contract, event: "TransferAdminRole", logs: logs, sub: sub}, nil
}

// WatchTransferAdminRole is a free log subscription operation binding the contract event 0xa5b56b7906fd0a20e3f35120dd8343db1e12e037a6c90111c7e42885e82a1ce6.
//
// Solidity: event TransferAdminRole(address newPendingAdmin)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchTransferAdminRole(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaTransferAdminRole) (event.Subscription, error) {

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "TransferAdminRole")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaTransferAdminRole)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "TransferAdminRole", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseTransferAdminRole is a log parse operation binding the contract event 0xa5b56b7906fd0a20e3f35120dd8343db1e12e037a6c90111c7e42885e82a1ce6.
//
// Solidity: event TransferAdminRole(address newPendingAdmin)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseTransferAdminRole(log types.Log) (*PolygonzkevmfeijoaTransferAdminRole, error) {
	event := new(PolygonzkevmfeijoaTransferAdminRole)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "TransferAdminRole", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}

// PolygonzkevmfeijoaVerifyBlobsIterator is returned from FilterVerifyBlobs and is used to iterate over the raw logs and unpacked data for VerifyBlobs events raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaVerifyBlobsIterator struct {
	Event *PolygonzkevmfeijoaVerifyBlobs // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *PolygonzkevmfeijoaVerifyBlobsIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(PolygonzkevmfeijoaVerifyBlobs)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(PolygonzkevmfeijoaVerifyBlobs)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *PolygonzkevmfeijoaVerifyBlobsIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *PolygonzkevmfeijoaVerifyBlobsIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// PolygonzkevmfeijoaVerifyBlobs represents a VerifyBlobs event raised by the Polygonzkevmfeijoa contract.
type PolygonzkevmfeijoaVerifyBlobs struct {
	SequneceNum uint64
	StateRoot   [32]byte
	Aggregator  common.Address
	Raw         types.Log // Blockchain specific contextual infos
}

// FilterVerifyBlobs is a free log retrieval operation binding the contract event 0xb19baa6f6271636400b99e9e5b3289ec1e0d74e6204a27f296cc4715ff9ded55.
//
// Solidity: event VerifyBlobs(uint64 indexed sequneceNum, bytes32 stateRoot, address indexed aggregator)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) FilterVerifyBlobs(opts *bind.FilterOpts, sequneceNum []uint64, aggregator []common.Address) (*PolygonzkevmfeijoaVerifyBlobsIterator, error) {

	var sequneceNumRule []interface{}
	for _, sequneceNumItem := range sequneceNum {
		sequneceNumRule = append(sequneceNumRule, sequneceNumItem)
	}

	var aggregatorRule []interface{}
	for _, aggregatorItem := range aggregator {
		aggregatorRule = append(aggregatorRule, aggregatorItem)
	}

	logs, sub, err := _Polygonzkevmfeijoa.contract.FilterLogs(opts, "VerifyBlobs", sequneceNumRule, aggregatorRule)
	if err != nil {
		return nil, err
	}
	return &PolygonzkevmfeijoaVerifyBlobsIterator{contract: _Polygonzkevmfeijoa.contract, event: "VerifyBlobs", logs: logs, sub: sub}, nil
}

// WatchVerifyBlobs is a free log subscription operation binding the contract event 0xb19baa6f6271636400b99e9e5b3289ec1e0d74e6204a27f296cc4715ff9ded55.
//
// Solidity: event VerifyBlobs(uint64 indexed sequneceNum, bytes32 stateRoot, address indexed aggregator)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) WatchVerifyBlobs(opts *bind.WatchOpts, sink chan<- *PolygonzkevmfeijoaVerifyBlobs, sequneceNum []uint64, aggregator []common.Address) (event.Subscription, error) {

	var sequneceNumRule []interface{}
	for _, sequneceNumItem := range sequneceNum {
		sequneceNumRule = append(sequneceNumRule, sequneceNumItem)
	}

	var aggregatorRule []interface{}
	for _, aggregatorItem := range aggregator {
		aggregatorRule = append(aggregatorRule, aggregatorItem)
	}

	logs, sub, err := _Polygonzkevmfeijoa.contract.WatchLogs(opts, "VerifyBlobs", sequneceNumRule, aggregatorRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(PolygonzkevmfeijoaVerifyBlobs)
				if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "VerifyBlobs", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseVerifyBlobs is a log parse operation binding the contract event 0xb19baa6f6271636400b99e9e5b3289ec1e0d74e6204a27f296cc4715ff9ded55.
//
// Solidity: event VerifyBlobs(uint64 indexed sequneceNum, bytes32 stateRoot, address indexed aggregator)
func (_Polygonzkevmfeijoa *PolygonzkevmfeijoaFilterer) ParseVerifyBlobs(log types.Log) (*PolygonzkevmfeijoaVerifyBlobs, error) {
	event := new(PolygonzkevmfeijoaVerifyBlobs)
	if err := _Polygonzkevmfeijoa.contract.UnpackLog(event, "VerifyBlobs", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}
